...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (actionType === "delete") {
    const { error } = await supabase.from(table.toString()).delete().eq("id", id.toString());
  ...
  if (actionType === "toggleStatus") {
    if (!status) {
      return Response.json({ message: "Status não fornecido" }, { status: 400 });
    }

    const { error } = await supabase
      .from(table.toString())
      .update({ status: status.toString() === "true" ? false : true })
      .eq("id", id.toString());
    ...
    return Response.json({ 
      ...
      newStatus: status.toString() === "true" ? false : true
    });
  }
...
export async function loader() {
  ...
  const totalReceitaHoje = receitaHojeData?.reduce((acc, venda) => acc + (venda.valor_total || 0), 0) || 0;
  ...
  return Response.json({ 
    estatisticas: {
      ...
      totalReceitaHoje: totalReceitaHoje || 0,
...
export default function Dashboard() {
    ...
    fetcher.submit(
      {
        ...
        table,
        id: id?.toString() || '',
        status: currentStatus?.toString() || 'false'
      },
      { method: "POST" }
    );
  };
  ...
  // Função segura para formatar valores em MZN
          ...
          {isMobile ? (
            <MobileTable
              ...
              renderItem={(produto) => (
                <div className="py-3 border-b">
                  <div className="flex justify-between">
                    <span className="font-medium">{produto?.nome || "Produto sem nome"}</span>
                    <span className="text-blue-600">{formatMZN(produto?.preco)}</span>
                  </div>
                  <div className="flex justify-between text-sm text-gray-600 mt-1">
                    <span>Qtd: {produto?.quantidade || 0}</span>
                      ...
                      {produto?.id && (
                        <>
                          ...
                          <fetcher.Form method="POST">
                            ...
                            <input type="hidden" name="id" value={produto.id.toString()} />
                            <input type="hidden" name="status" value={produto.status?.toString() || 'false'} />
                            <button
                              type="submit"
                              className="text-gray-600 text-sm flex items-center gap-1"
                ...
                <tbody className="divide-y divide-gray-200">
                  {recentProdutos.map((produto) => (
                    <tr key={produto?.id || Math.random()} className="hover:bg-gray-50">
                      <td className="p-3 whitespace-nowrap">
                        <div className="font-medium">{produto?.nome || "Produto sem nome"}</div>
                        <div className="text-xs text-gray-500">{produto?.fabricante || "Fabricante não especificado"}</div>
                      </td>
          ...
          {isMobile ? (
            <MobileTable
              ...
              renderItem={(usuario) => (
                <div className="py-3 border-b">
                  <div className="flex justify-between">
                    <span className="font-medium">{usuario?.nome || "Usuário sem nome"}</span>
                    <span className="text-gray-600">{translateRole(usuario?.role)}</span>
                  </div>
                  <div className="text-sm text-gray-600 mt-1">{usuario?.email || "Sem email"}</div>
                  <div className="mt-2 flex justify-between">
                      ...
                      {usuario?.id && (
                        <>
                          ...
                          <fetcher.Form method="POST">
                            ...
                            <input type="hidden" name="id" value={usuario.id.toString()} />
                            <input type="hidden" name="status" value={usuario.status?.toString() || 'false'} />
                            <button
                              type="submit"
                              className="text-gray-600 text-sm flex items-center gap-1"
